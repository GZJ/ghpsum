...
def main():
    parser = argparse.ArgumentParser(description='Fetch GitHub repositories and their descriptions.')
    parser.add_argument('--username', type=str, required=True, help='GitHub username')
    parser.add_argument('--filter-fork', action='store_true', help='Filter out forked repositories')
    parser.add_argument('--filter-github-io', action='store_true', help='Filter out repositories containing github.io')
    parser.add_argument('--output-format', type=str, choices=['csv', 'json', 'html'], required=True, help='Output file format: csv, json or html')

    args = parser.parse_args()

    repo_data = get_repositories(args.username, args.filter_fork, args.filter_github_io)

    output_file = f'{args.username}_github_repositories.{args.output_format}'
    if args.output_format == 'csv':
        df = pd.DataFrame(repo_data)
        df.to_csv(output_file, index=False)
        print(f"Repository names and descriptions of {args.username}'s projects have been saved to {output_file}")
